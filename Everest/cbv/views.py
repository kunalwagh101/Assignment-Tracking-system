...
class StudentView(ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if context["action"] == "update":
            update_assignments = [{"assignment": assignment, "ontime": True if assignment.is_ontime() else False} for assignment in Submission.Submited_assignment(self.request.user)]
            paginator = Paginator(update_assignments, self.paginate_by)
        else:
            submit_assignments = [{"assignment": assignment, "is_submit": assignment.check_submit(self.request.user),
                                    "check_deadline": True if str(assignment.deadline) >= str(formatted_now) else False} for assignment in context["assignments"]]
            paginator = Paginator(submit_assignments, self.paginate_by)
